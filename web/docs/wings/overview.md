# Wings

## Minimum Requirements

Before installing the Calagopus Wings Daemon, ensure that your system meets the following minimum requirements:

- **Operating System**: Ubuntu 22.04 LTS or later, Debian 11 or later, or anything that supports modern Docker versions
- **CPU Architecture**: x86_64, ARM64, RISC-V or PPC64LE
- **RAM**: Minimum 512 MB (1 GB or more recommended)
- **Disk Space**: At least 512 MB of free disk space

**Important:** Wings requires Docker to be installed and running on the host machine to manage game server containers, these minimum requirements also do not inlude actual load from game servers which will require additional resources based on the games being hosted.
