...
::: warning
Using `fuse_quota` with a Docker Wings installation is **strongly** discouraged. Every time Wings restarts, Fusequota is killed and the server filesystem mounts become inaccessible, causing I/O errors until all servers are manually restarted.
:::
...
- **Stability.** FUSE as a technology has a reputation for occasional instability (stuck mounts, kernel/user-space sync issues under load). Fusequota is designed to be as robust as possible, but it is fundamentally less stable than a kernel-level filesystem. Monitor your servers after rolling it out.
...
If you are unsure which limiter is right for your setup, ask in the [Discord](https://discord.gg/uSM8tvTxBV) for guidance.
