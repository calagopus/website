...
- [**Btrfs Subvolume**](./btrfs-subvolume.md): A native, kernel-enforced disk limiter using Btrfs subvolumes and qgroups. Stable and performant, but requires Btrfs. Existing servers can be converted with the `migrate-disk-limiter` command.
- [**ZFS Dataset**](./zfs-dataset.md): A native, kernel-enforced disk limiter using ZFS datasets and `refquota`. Stable and performant, but requires ZFS. Existing servers can be converted with the `migrate-disk-limiter` command.
