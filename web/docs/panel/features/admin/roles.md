...
| **Require Two Factor** | "Require users with this role to use two factor authentication." |
...
![Role form with permission pickers](./images/roles/edit.webp)
...
A role's view has a **Users** tab (requires `users.read`) listing every user currently holding the role, with the same columns as the [Users](./users.md) list.
