...
| **Allocation Mode** | How allocations map onto the destination: none, randomize primary, randomize all, preserve port numbers (default), or assign from the egg's deployment configuration or port range. Each option's dropdown entry explains its caveats. |
...
| **Archive Format** / **Compression Level** | How the server data is packed for the move. |
