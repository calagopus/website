...
The list shows ID, **Order**, Name, **Eggs** (how many eggs the configuration contains), and Created.

## How Configurations Stack

An egg can appear in multiple configurations. All configurations containing a server's egg apply, sorted by their **Order** value (lowest first, creation date as tie-breaker), and each section (allocation, startup, page order) is taken from the last configuration in that ordering that defines it. In practice: put your broad defaults at a low Order and targeted overrides at a higher one.

## Creating and Editing

Click **Create**, or open a configuration's ID to edit it. The base fields are **Name**, **Order**, **Eggs** (a multi-select grouped by nest), and **Description**. If your panel has no eggs yet, a warning tells you to create a nest and egg first.

![](./images/egg-configurations/general-form.webp)
...
![](./images/egg-configurations/allocation.webp)
...
Reorders the server sidebar for affected servers. Drag pages into the order you want, insert **Divider** entries to group them, or add **Redirect** entries (a name plus a destination URL) for external links.
