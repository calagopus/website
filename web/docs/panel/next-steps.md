# Next steps
Congratulations! You have now installed Calagopus Panel in your server. All that's left is to start the OOBE, and you should be all set!

## What's next?
The following guides below will show you some steps that you may be interested in:

### Creating a new node
Please see the [Creating a new node](./next-steps/add-node.md) documentation in the Next Steps part of the documentation.

### Adding egg repositories
Please see the [Adding egg repositories](./next-steps/egg-repos.md) documentation in the Next Steps part of the documentation.